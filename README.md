...
grug is a general-purpose embedded programming language with one job: make your mods immortal. grug's vision is that a mod written today will continue to run 100 years from now, regardless of how much around it changes. A Minecraft mod written in grug aims to run in any version of the game, from Beta 1.7.3 (2011) to the latest, with no manual changes to its source code whatsoever.
